...
$(function () {
    ...
    $(".contForm").submit(function () {
        ...
        var phoneFilter = /[0-9]+$/;
        var phoneText = $(".phone").val().length < 12;
        ...
        if ((captchaVal !== captchaCode) || (!emailFilter.test(emailText)) || (!phoneFilter.test(phoneText)) || (!nameFilter.test(nameText)) || (messageText < 20)) {
            return false;
        }
        if ((captchaVal === captchaCode) && (emailFilter.test(emailText)) && (phoneFilter.test(phoneText)) && (nameFilter.test(nameText)) && (messageText > 20)) {
            $(".contForm").css("display", "none");
